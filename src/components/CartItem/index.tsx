...
import { addProduct, minusOneItem, removeItem } from '../../redux/slice/cartSlice';
...
const CartItem: React.FC<ItemCart> = ({ guid, title, count, price, imageUrl, typeName, sizeName, }) => {

    const dispatch = useDispatch();

    function onClickPlus() {
        dispatch(addProduct({ guid }));
    }

    function onClickMinus() {
        dispatch(minusOneItem({ guid }));
    }

    function onClickRemoveItem() {
        dispatch(removeItem({ guid }));
    }
        ...
        <>
            <div className="cart__item">
                ...
                <div className="cart__item-info">
                    <h3>{title}</h3>
                    <p>size: {sizeName} - type: {typeName}</p>
                </div>
